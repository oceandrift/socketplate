...
@safe:
    ...
    /++
     ...
     +/
    T[] receiveSlice(T)(return scope T[] buffer) if (is(T == void) || is(T == ubyte) || is(T == char)) {
    ...
    /++
     ...
     +/
    T[] receiveAll(T)(return scope T[] buffer) if (is(T == void) || is(T == ubyte) || is(T == char)) {
    ...
    ///
    long timeout(Direction direction)() if (direction == Direction.send || direction == Direction.receive) {
...
private {
    long getTimeout(Direction direction)(Socket socket) if (direction == Direction.send || direction == Direction.receive) {
